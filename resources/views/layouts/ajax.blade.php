...
$(document).on('change', '#group_id', function() {
    ...
    response1 = getData("/ims/crud/get", "SubGroup", where, 'get', []);
    ...
    response2 = getData("/ims/crud/get", "Item", where, 'get', []);
...
$(document).on('change', '#subgroup_id', function() {
    ...
    response = getData("/ims/crud/get", "SubGroupVolume", where, 'get', []);
    if (response) {
...
function dataExist(table, where) {
    let response = '';
    url = window.base+'/admin/crud/exist';
    $.ajax({
            async: false,
            headers: {
                'x-csrf-token': _token
...
function presentStock(branch_id, warehouse_id, group_id, subgroup_id, item_id, from_date, to_date) {
    let response = '';
    url = window.base+'/admin/inventory_management/item/present-stock';
    $.ajax({
            async: false,
            headers: {
                'x-csrf-token': _token
...
function presentToolsStock(branch_id, warehouse_id, group_id, subgroup_id, item_id, from_date, to_date) {
    let response = '';
    url = window.base+'/admin/inventory_management/item/present-tools-stock';
    $.ajax({
            async: false,
            headers: {
                'x-csrf-token': _token
...
function GenerateCode() {
    ...
    if (update_id == 0) {
        ...
        url = '/ims/admin/crud/generate-code';
        $.ajax({
                async: false,
                headers: {
                    'x-csrf-token': _token
...
function checkDuplicate() {
    ...
    $(element).blur(function() {
        ...
        response = getData("/ims/crud/get", model, where, 'get', []);
