...
<script>
    $(document).on('change','#group_id,#subgroup_id,#item_id',function(){
       ...
       page = getPage(window.base+"/admin/inventory_management/opening/get-item","Item",where,"admin.opening.item_list");
       $('#content').html(page);
    })
    ...
    url = window.base+"/admin/inventory_management/opening/item-store";
    $.ajax({
            async: false,
            headers: {
                'x-csrf-token': _token
