...
class RequisitionController extends Controller
{
    public function index()
    {
        ...
        $user = auth()->user() ;
        $where = [] ;
        if($user->role_id == UserRole::DEPARTMENT){
            $where[] = ['section_id','=',$user->section_id];
        }

        $requisitions = Requisition::where($where)->orderBy('id', 'desc')->get();
    ...
    public function store(RequisitionStoreRequest $request)
    {
        ...
        return redirect()->route('admin.requisition.index')->with("message", $message);
    }
